...
# Split data ensuring last 4 blends are in testing, remaining 17 of last 21 in training
print("Splitting data with 80/20 split, ensuring last 4 blends in testing, remaining 17 of last 21 in training...")

last_21_indices = list(range(len(df) - 21, len(df)))
last_4_indices = list(range(len(df) - 4, len(df)))  # Last 4 go to testing
last_17_indices = list(range(len(df) - 21, len(df) - 4))  # Remaining 17 of last 21 go to training
...
# Combine test indices: remaining test data + last 4 blends
...
print(f"Last 4 blend indices (testing): {last_4_indices}")
...
last_4_indices_from_21 = last_21_indices[-4:]
last_4_X = X.iloc[last_4_indices_from_21]
last_4_y = y.iloc[last_4_indices_from_21]
