...
plt.subplot(1, 2, 1)
plt.scatter(last_5_y, last_5_pred, color='red', s=100, alpha=0.7)
...
plt.title('WVTR Predictions on erthos Validation')
...
plt.scatter(original_actual_5/100, original_pred_5/100, color='blue', s=100, alpha=0.7)
plt.plot([original_actual_5.min()/100, original_actual_5.max()/100], [original_actual_5.min()/100, original_actual_5.max()/100], 'r--', lw=2)
plt.xlabel('Actual Property (100um thickness)')
plt.ylabel('Predicted Property (100um thickness)')
plt.title('WVTR Predictions on erthos Validation')

# Add metrics as text box on the plot
metrics_text = (
    f'MAE: {original_mae_5/100:.2f} (100um thickness)\n'
    f'R²: {original_r2_5:.2f}'
)
plt.gca().text(0.05, 0.95, metrics_text, transform=plt.gca().transAxes,
                fontsize=12, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

# Add individual data point labels
for i, (actual, pred) in enumerate(zip(original_actual_5, original_pred_5)):
    plt.annotate(f'{actual/100:.1f}', (actual/100, pred/100), textcoords="offset points", xytext=(0,10), ha='center', fontsize=8)

plt.tight_layout()
plt.savefig(os.path.join(args.output, 'last_5_blends_performance.png'), dpi=300, bbox_inches='tight')
plt.close()

# Calculate original scale metrics for last 5 blends
original_mae_5 = mean_absolute_error(original_actual_5, original_pred_5)
...
print("✅ Last 5 blends performance plot saved as 'last_5_blends_performance.png'")
