...
y1 = df[target_col1]  # max_L property
y2 = df[target_col2]  # t0 property
...
# Create comprehensive results plot for both models
plt.figure(figsize=(20, 12))

# Plot 1: max_L model performance (log scale)
plt.subplot(2, 2, 1)
plt.scatter(log_y1_train, model1.predict(X_train), color='blue', s=50, alpha=0.6, label='Training')
plt.scatter(log_y1_test, model1.predict(X_test), color='red', s=50, alpha=0.6, label='Testing')
plt.plot([log_y1.min(), log_y1.max()], [log_y1.min(), log_y1.max()], 'k--', lw=2)
plt.xlabel('Actual Log(max_L)')
plt.ylabel('Predicted Log(max_L)')
plt.title('max_L Model Performance (Log Scale)')
plt.legend()
plt.grid(True, alpha=0.3)

# Plot 2: t0 model performance (log scale)
plt.subplot(2, 2, 2)
plt.scatter(log_y2_train, model2.predict(X_train), color='blue', s=50, alpha=0.6, label='Training')
plt.scatter(log_y2_test, model2.predict(X_test), color='red', s=50, alpha=0.6, label='Testing')
plt.plot([log_y2.min(), log_y2.max()], [log_y2.min(), log_y2.max()], 'k--', lw=2)
plt.xlabel('Actual Log(t0)')
plt.ylabel('Predicted Log(t0)')
plt.title('t0 Model Performance (Log Scale)')
plt.legend()
plt.grid(True, alpha=0.3)

# Plot 3: max_L model performance (original scale)
plt.subplot(2, 2, 3)
y1_train_pred = np.exp(model1.predict(X_train)) - 1e-6
y1_test_pred = np.exp(model1.predict(X_test)) - 1e-6
y1_train_actual = np.exp(log_y1_train) - 1e-6
y1_test_actual = np.exp(log_y1_test) - 1e-6

plt.scatter(y1_train_actual, y1_train_pred, color='blue', s=50, alpha=0.6, label='Training')
plt.scatter(y1_test_actual, y1_test_pred, color='red', s=50, alpha=0.6, label='Testing')
plt.plot([y1.min(), y1.max()], [y1.min(), y1.max()], 'k--', lw=2)
plt.xlabel('Actual max_L')
plt.ylabel('Predicted max_L')
plt.title('max_L Model Performance (Original Scale)')
plt.legend()
plt.grid(True, alpha=0.3)

# Plot 4: t0 model performance (original scale)
plt.subplot(2, 2, 4)
y2_train_pred = np.exp(model2.predict(X_train))
y2_test_pred = np.exp(model2.predict(X_test))
y2_train_actual = np.exp(log_y2_train)
y2_test_actual = np.exp(log_y2_test)

plt.scatter(y2_train_actual, y2_train_pred, color='blue', s=50, alpha=0.6, label='Training')
plt.scatter(y2_test_actual, y2_test_pred, color='red', s=50, alpha=0.6, label='Testing')
plt.plot([y2.min(), y2.max()], [y2.min(), y2.max()], 'k--', lw=2)
plt.xlabel('Actual t0 (days)')
plt.ylabel('Predicted t0 (days)')
plt.title('t0 Model Performance (Original Scale)')
plt.legend()
plt.grid(True, alpha=0.3)
...
print("✅ Comprehensive results plot saved as 'comprehensive_polymer_model_results.png'")
