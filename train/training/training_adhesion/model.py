...
parser = argparse.ArgumentParser(description='Train XGBoost model for Adhesion polymer blends (v1 - Memorization Model)')
parser.add_argument('--input', type=str, default='training/data/adhesion/polymerblends_for_ml_featurized.csv',
                    help='Input CSV file path (default: training/data/adhesion/polymerblends_for_ml_featurized.csv)')
parser.add_argument('--output', type=str, default='models/adhesion/v1',
                    help='Output directory path (default: models/adhesion/v1)')
parser.add_argument('--target', type=str, default='property',
                    help='Target column name (default: property)')
parser.add_argument('--last_n_in_testing', type=int, default=5,
                    help='Number of last N blends to put in testing set (default: 5)')
...
# Split data ensuring last N blends are in testing with 80/20 split
print(f"Splitting data with 80/20 split ensuring last {args.last_n_in_testing} blends are in testing...")
...
test_mask[last_n_in_testing_indices] = True
train_mask[last_n_in_testing_indices] = False

# Use train_test_split on the remaining data (excluding last N)
remaining_indices = list(range(len(df) - args.last_n_in_testing))
...
print(f"Last {args.last_n_in_testing} blends in testing: {all(i in np.where(test_mask)[0] for i in last_n_in_testing_indices)}")

print("\nTraining the XGBoost model for Adhesion prediction (v1 - Memorization Model)...")
...
plt.title(f'Top {top_features} Feature Importances (v1 - Adhesion Memorization Model)')
...
# Create separate plot for last N blends performance (in testing set)
print(f"\n=== LAST {args.last_n_in_testing} BLENDS PERFORMANCE (IN TESTING) ===")
...
print(f"Last {args.last_n_in_testing} Blends Performance (Testing Set):")
print(f"R² Score: {last_n_r2:.4f}")
print(f"Mean Absolute Error: {last_n_mae:.4f}")
print(f"Root Mean Squared Error: {last_n_rmse:.4f}")

# Create separate plot for last N blends - Original Scale Only
plt.figure(figsize=(10, 8))

# Convert back to original scale
original_actual = np.exp(last_n_y)
original_pred = np.exp(last_n_pred)
...
plt.xlabel('Actual Adhesion (Original Scale)')
plt.ylabel('Predicted Adhesion (Original Scale)')
plt.title('Testing Adhesion Strength Models on Literature Blends')

# Add metrics as text box on the plot
metrics_text = (
    f'MAE: {original_mae:.2f}\n'
    f'RMSE: {original_rmse:.2f}'
)
...
for i, (actual, pred) in enumerate(zip(original_actual, original_pred)):
    ...
    blend_info = f"{actual:.1f}\n{', '.join(grades)}\n({', '.join(vol_fractions)})"
    
    plt.annotate(blend_info, (actual, pred), textcoords="offset points", xytext=(0,15), 
...
print(f"\nLast {args.last_n_in_testing} Blends - Original Scale Performance (Testing Set):")
...
plt.tight_layout()
plt.savefig(os.path.join(args.output, f'last_{args.last_n_in_testing}_blends_detailed.png'), dpi=300, bbox_inches='tight')
plt.close()
...
def predict_adhesion_property(input_data):
    """
    Predict Adhesion property for polymer blend data (Memorization Model - High Training Performance)
    
    Parameters:
    - input_data: pandas DataFrame with all features (excluding Materials column)
    
    Returns:
    - float: predicted Adhesion property
    """
    # Handle missing values
    for col in categorical_features:
...
predicted_value = predict_adhesion_property(example_data)
