...
# Oversample the last 4 blends by 2x
last_4_X = X.iloc[last_4_indices]
last_4_log_y1 = log_y1.iloc[last_4_indices]
last_4_log_y2 = log_y2.iloc[last_4_indices]

# Repeat the last 4 blends 1 more time (total 2x)
oversampled_X = []
oversampled_y1 = []
oversampled_y2 = []

for _ in range(1):  # Repeat 1 more time (total 2x)
    oversampled_X.append(last_4_X)
    oversampled_y1.append(last_4_log_y1)
    oversampled_y2.append(last_4_log_y2)

# Combine original training data with oversampled last 4
X_train_final = pd.concat([X_train] + oversampled_X, ignore_index=True)
log_y1_train_final = pd.concat([log_y1_train] + oversampled_y1, ignore_index=True)
log_y2_train_final = pd.concat([log_y2_train] + oversampled_y2, ignore_index=True)
...
model1.fit(X_train_final, log_y1_train_final)

print("Training EAB2 model...")
model2.fit(X_train_final, log_y2_train_final)
