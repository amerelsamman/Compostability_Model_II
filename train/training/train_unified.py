...
def create_comprehensive_plots(models: List[Pipeline], X_train: pd.DataFrame, X_test: pd.DataFrame,
    ...
    for i, (model, target_col) in enumerate(zip(models, target_cols)):
        ...
        axes[0, i].scatter(y_train, y_pred_train, alpha=0.6, color='blue')
        axes[0, i].plot([y_train.min(), y_train.max()], [y_train.min(), y_train.max()], 'r--', lw=2)
        axes[0, i].set_xlabel(f'Actual Log({target_col})')
        axes[0, i].set_ylabel(f'Predicted Log({target_col})')
        axes[0, i].set_title(f'{target_col} - Training Set')
        axes[0, i].text(0.05, 0.95, f'R²: {results[f"model_{i+1}"]["log_scale"]["train_r2"]:.4f}', 
        ...
        axes[1, i].scatter(y_test, y_pred_test, alpha=0.6, color='red')
        axes[1, i].plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'r--', lw=2)
        axes[1, i].set_xlabel(f'Actual Log({target_col})')
        axes[1, i].set_ylabel(f'Predicted Log({target_col})')
        axes[1, i].set_title(f'{target_col} - Test Set')
        axes[1, i].text(0.05, 0.95, f'R²: {results[f"model_{i+1}"]["log_scale"]["test_r2"]:.4f}', 
