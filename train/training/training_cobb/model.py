...
parser = argparse.ArgumentParser(description='Train XGBoost model for Cobb polymer blends (v1 - Memorization Model)')
...
# Split data ensuring last 10 blends are in training with 80/20 split
print("Splitting data with 80/20 split ensuring last 10 blends in training...")
...
# Set last 10 blends to training
train_mask[last_10_indices] = True
...
print(f"Last 10 blends in training: {all(i in np.where(train_mask)[0] for i in last_10_indices)}")

print("\nTraining the XGBoost model for Cobb prediction (v1 - Memorization Model)...")
...
plt.title(f'Top {top_features} Feature Importances (v1 - Cobb Memorization Model)')
...
plt.title('Cobb Predictions - Original Scale (v1 - Memorization Model)')
...
# Create separate plot for last 1 blend performance
print("\n=== LAST 1 BLEND PERFORMANCE ===")
# Get predictions for the very last blend
last_1_index = len(df) - 1
last_1_X = X.iloc[[last_1_index]]
last_1_y = y.iloc[last_1_index]
last_1_pred = model.predict(last_1_X)[0]

last_1_mae = abs(last_1_y - last_1_pred)
last_1_r2 = 1.0  # R² for single point is always 1.0

print(f"Last 1 Blend Performance:")
print(f"Actual Log(Property): {last_1_y:.4f}")
print(f"Predicted Log(Property): {last_1_pred:.4f}")
print(f"Absolute Error: {last_1_mae:.4f}")
...
# Plot 1: Log-transformed predictions
plt.subplot(1, 2, 1)
plt.scatter(last_1_y, last_1_pred, color='red', s=100, alpha=0.7)
plt.plot([last_1_y - 0.5, last_1_y + 0.5], [last_1_y - 0.5, last_1_y + 0.5], 'k--', lw=2)
plt.xlabel('Actual Log(Property)')
plt.ylabel('Predicted Log(Property)')
plt.title('COBB Predictions on erthos Validation')
...
# Add individual data point label
plt.annotate(f'{last_1_y:.1f}', (last_1_y, last_1_pred), textcoords="offset points", xytext=(0,10), ha='center', fontsize=8)

# Plot 2: Original scale predictions
plt.subplot(1, 2, 2)
original_last_actual = np.exp(last_1_y)
original_last_pred = np.exp(last_1_pred)
original_mae = abs(original_last_actual - original_last_pred)
original_r2 = 1.0  # R² for single point is always 1.0

plt.scatter(original_last_actual, original_last_pred, color='blue', s=100, alpha=0.7)
...
# Add individual data point label
plt.annotate(f'{original_last_actual:.1f}', (original_last_actual, original_last_pred), textcoords="offset points", xytext=(0,10), ha='center', fontsize=8)

plt.tight_layout()
plt.savefig(os.path.join(args.output, 'last_1_blends_performance.png'), dpi=300, bbox_inches='tight')
plt.close()

print(f"\nLast 1 Blend - Original Scale Performance:")
print(f"Actual Property: {original_last_actual:.2f}")
print(f"Predicted Property: {original_last_pred:.2f}")
print(f"Absolute Error: {original_mae:.2f}")
print("✅ Last 1 blend performance plot saved as 'last_1_blends_performance.png'")
...
def predict_cobb_property(input_data):
    """
    Predict Cobb property for polymer blend data (Memorization Model - High Training Performance)
