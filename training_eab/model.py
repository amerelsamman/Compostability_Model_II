...
# Split the data - ensure last 15 blends are in training, rest split 80/20
...
# Always include last 15 in training
train_mask[last_15_indices] = True

# Use train_test_split on the rest
remaining_indices = list(range(len(df) - 15))
...
print(f"Last 15 blends in training: {all(i in np.where(train_mask)[0] for i in last_15_indices)}")

# Oversample the last 4 blends 10x
print("\nOversampling last 4 blends 10x...")
last_4_indices = list(range(len(df) - 4, len(df)))
print(f"Last 4 blend indices: {last_4_indices}")

# Create oversampling weights - set all to 1.0, then 10x for last 4
