#!/usr/bin/env python3
"""
TS Blend Prediction Script
Takes polymer names, grades, volume fractions, and thickness as command line inputs
and predicts tensile strength using the trained v1 model.

Usage: python predict_ts_blend.py "PLA, 4032D, 0.5, PBAT, Ecoworld, 0.5, 100"
...
def load_ts_model(model_path='models/ts/v1/comprehensive_polymer_model.pkl'):
    """Load the trained TS model."""
    try:
        model = joblib.load(model_path)
        logger.info(f"✅ TS model loaded successfully from {model_path}")
        return model
    except Exception as e:
        logger.error(f"❌ Error loading TS model: {e}")
        return None
...
def predict_ts(features_df, model):
    """
    Make TS predictions using the trained model.
    ...
    try:
        ...
        logger.info(f"✅ TS prediction made successfully")
        return prediction_log
        
    except Exception as e:
        logger.error(f"❌ Error making TS prediction: {e}")
        return None

def main():
    """Main function to run the TS blend prediction."""
    parser = argparse.ArgumentParser(description='Predict tensile strength for polymer blends')
    parser.add_argument('input', type=str, 
                       help='Input string: "Material1, Grade1, vol_fraction1, Material2, Grade2, vol_fraction2, ..., Thickness"')
    parser.add_argument('--model', type=str, default='models/ts/v1/comprehensive_polymer_model.pkl',
                       help='Path to TS model (default: models/ts/v1/comprehensive_polymer_model.pkl)')
    ...
    print("=== TS BLEND PREDICTION SCRIPT ===")
    ...
    try:
        ...
        # Load TS model
        print("\n=== LOADING TS MODEL ===")
        model = load_ts_model(args.model)
        if model is None:
            return
        ...
        # Make prediction
        print("\n=== MAKING TS PREDICTION ===")
        prediction_log = predict_ts(features_df, model)
        ...
        print(f"Original-scale prediction: {prediction_original:.2f} MPa")
