...
oversampled_y.append(log_y.iloc[other_train_indices])

# Add last 4 blends 10 times
for _ in range(10):
    oversampled_X.append(last_4_X)
    oversampled_y.append(last_4_log_y)

# Combine all data
X_train_oversampled = pd.concat(oversampled_X, ignore_index=True)
log_y_train_oversampled = pd.concat(oversampled_y, ignore_index=True)

print(f"Original training set size: {X_train.shape[0]}")
print(f"Oversampled training set size: {X_train_oversampled.shape[0]}")
print(f"Last 4 blends repeated 10x: {last_4_X.shape[0] * 10} samples")
...
def predict_tensile_strength(input_data):
    """
    Predict tensile strength for polymer blend data 
    Parameters:
...
with open(os.path.join(args.output, 'training_explanation.txt'), 'w') as f:
    ...
    f.write(f"Training set: {len(orig_y_train)} samples (including last 4 blends oversampled 10x)\n")
