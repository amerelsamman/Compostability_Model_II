#!/usr/bin/env python3
"""
EAB Blend Prediction Script
Takes polymer names, grades, volume fractions, and thickness as command line inputs
and predicts elongation at break using the trained v1 model.

Usage: python predict_eab_blend.py "PLA, 4032D, 0.5, PBAT, Ecoworld, 0.5, 100"

Format: "Material1, Grade1, vol_fraction1, Material2, Grade2, vol_fraction2, ..., Thickness"
"""
...
def parse_command_line_input(input_string):
    ...
    Expected format: "Material1, Grade1, vol_fraction1, Material2, Grade2, vol_fraction2, ..., Thickness"
    ...
    try:
        ...
        if len(parts) < 4:  # Minimum: 2 polymers (6 parts) + 1 thickness
            raise ValueError("Input must have at least 2 polymers and 1 thickness parameter")
        
        # Extract thickness (last value)
        thickness = float(parts[-1])

        # Extract polymer information (everything except last 1)
...
def create_input_dataframe(smiles_list, thickness, polymers):
    """
...
def load_eab_model(model_path='models/eab/v1/comprehensive_polymer_model.pkl'):
    """Load the trained EAB model."""
    try:
        model = joblib.load(model_path)
        logger.info(f"✅ EAB model loaded successfully from {model_path}")
        return model
    except Exception as e:
        logger.error(f"❌ Error loading EAB model: {e}")
        return None

def prepare_features_for_prediction(featurized_df, model):
    """
    Prepare featurized data exactly as expected by the trained TS model.
...
def predict_eab(features_df, model):
    """
    Make EAB predictions using the trained model.
    ...
    try:
        ...
        logger.info(f"✅ EAB prediction made successfully")
        return prediction_log
        
    except Exception as e:
        logger.error(f"❌ Error making EAB prediction: {e}")
        return None

def main():
    """Main function to run the EAB blend prediction."""
    parser = argparse.ArgumentParser(description='Predict elongation at break for polymer blends')
    parser.add_argument('input', type=str, 
                       help='Input string: "Material1, Grade1, vol_fraction1, Material2, Grade2, vol_fraction2, ..., Thickness"')
    parser.add_argument('--model', type=str, default='models/eab/v1/comprehensive_polymer_model.pkl',
                       help='Path to EAB model (default: models/eab/v1/comprehensive_polymer_model.pkl)')
    ...
    print("=== EAB BLEND PREDICTION SCRIPT ===")
    ...
    print("\n=== PARSING INPUT ===")
    polymers, thickness = parse_command_line_input(args.input)
    if polymers is None:
        return
    ...
    input_df = create_input_dataframe(smiles_list, thickness, polymers)
    if input_df is None:
        return
    ...
    try:
        ...
        # Load EAB model
        print("\n=== LOADING EAB MODEL ===")
        model = load_eab_model(args.model)
        if model is None:
            return
        ...
        # Make prediction
        print("\n=== MAKING EAB PREDICTION ===")
        prediction_log = predict_eab(features_df, model)
        ...
        # Print results
        print("\n=== EAB PREDICTION RESULTS ===")
        print(f"Log-scale prediction: {prediction_log:.4f}")
        print(f"Original-scale prediction: {prediction_original:.2f} %")
        ...
        print(f"Environmental features: Thickness")
        print(f"Molecular features: {len(features_df.columns) - 1} descriptors")
