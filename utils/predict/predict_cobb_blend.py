#!/usr/bin/env python3
"""
Cobb Blend Prediction Script
Takes polymer names, grades, and volume fractions as command line inputs
and predicts Cobb value using the trained v1 model.

Usage: python predict_cobb_blend.py "PLA, 4032D, 0.5, PBAT, Ecoworld, 0.5"

Format: "Material1, Grade1, vol_fraction1, Material2, Grade2, vol_fraction2, ..."
"""
...
def parse_command_line_input(input_string):
    ...
    Expected format: "Material1, Grade1, vol_fraction1, Material2, Grade2, vol_fraction2, ..."
    ...
    try:
        ...
        if len(parts) < 3:  # Minimum: 1 polymer (3 parts)
            raise ValueError("Input must have at least 1 polymer")
...
def create_input_dataframe(smiles_list, polymers):
    """
...
def load_cobb_model(model_path='models/cobb/v1/comprehensive_polymer_model.pkl'):
    """Load the trained Cobb model."""
    try:
        model = joblib.load(model_path)
        logger.info(f"✅ Cobb model loaded successfully from {model_path}")
        return model
    except Exception as e:
        logger.error(f"❌ Error loading Cobb model: {e}")
        return None
...
def predict_cobb(features_df, model):
    """
    Make Cobb predictions using the trained model.
    ...
    try:
        ...
        logger.info(f"✅ Cobb prediction made successfully")
        return prediction_log
        
    except Exception as e:
        logger.error(f"❌ Error making Cobb prediction: {e}")
        return None

def main():
    """Main function to run the Cobb blend prediction."""
    parser = argparse.ArgumentParser(description='Predict Cobb value for polymer blends')
    parser.add_argument('input', type=str, 
                       help='Input string: "Material1, Grade1, vol_fraction1, Material2, Grade2, vol_fraction2, ..."')
    parser.add_argument('--model', type=str, default='models/cobb/v1/comprehensive_polymer_model.pkl',
                       help='Path to Cobb model (default: models/cobb/v1/comprehensive_polymer_model.pkl)')
    ...
    print("=== COBB BLEND PREDICTION SCRIPT ===")
    ...
    print("\n=== PARSING INPUT ===")
    polymers = parse_command_line_input(args.input)
    if polymers is None:
        return
    ...
    input_df = create_input_dataframe(smiles_list, polymers)
    if input_df is None:
        return
    ...
    try:
        ...
        # Load Cobb model
        print("\n=== LOADING COBB MODEL ===")
        model = load_cobb_model(args.model)
        if model is None:
            return
        ...
        # Make prediction
        print("\n=== MAKING COBB PREDICTION ===")
        prediction_log = predict_cobb(features_df, model)
        ...
        # Print results
        print("\n=== COBB PREDICTION RESULTS ===")
        print(f"Log-scale prediction: {prediction_log:.4f}")
        print(f"Original-scale prediction: {prediction_original:.2f} g/m²/60s")
        ...
        print(f"Molecular features: {len(features_df.columns)} descriptors")
