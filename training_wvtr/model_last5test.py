...
# Split data ensuring last 5 blends are in testing, remaining 16 of last 21 in training
print("Splitting data with 80/20 split, ensuring last 5 blends in testing, remaining 16 of last 21 in training...")

last_21_indices = list(range(len(df) - 21, len(df)))
last_5_indices = list(range(len(df) - 5, len(df)))  # Last 5 go to testing
last_16_indices = list(range(len(df) - 21, len(df) - 5))  # Remaining 16 of last 21 go to training
...
# Combine training indices: remaining training data + last 16 blends
train_indices = train_remaining + last_16_indices
...
print(f"Last 16 blend indices (training): {last_16_indices}")
...
print(f"Last 16 blends in training: {all(i in train_indices for i in last_16_indices)}")
...
last_5_indices_from_21 = last_21_indices[-5:]
last_5_X = X.iloc[last_5_indices_from_21]
last_5_y = y.iloc[last_5_indices_from_21]
