...
# Split data ensuring last 4 blends are in testing, last 17 blends in training
print("Splitting data ensuring last 4 blends in testing, last 17 blends in training...")

last_21_indices = list(range(len(df) - 21, len(df)))
last_4_indices = list(range(len(df) - 4, len(df)))  # Last 4 for testing
last_17_indices = list(range(len(df) - 21, len(df) - 4))  # Last 17-21 for training
...
# Combine indices: remaining training data + last 17 blends for training
# remaining test data + last 4 blends for testing
train_indices = train_remaining + last_17_indices
test_indices = test_remaining + last_4_indices
...
print(f"Last 17 blend indices (for training): {last_17_indices}")
...
# Oversample last 17 blends in training set by 5x (these are the last 17-21 blends)
# Since indices are reset after split, we need to identify the last 17 blends differently
# Get the last 17 samples from the training set (which should be the last 17-21 blends)
last_17_train_indices = list(range(len(X_train) - 17, len(X_train)))
print(f"Oversampling last 17 training samples (indices {last_17_train_indices}) by 5x")

for idx in last_17_train_indices:
    for _ in range(5):
...
plt.xlabel('Actual Log(Property)')
plt.ylabel('Predicted Log(Property)')
plt.title('Last 4 Blends: Log-Transformed Predictions (Memorization Model)')
plt.legend()
plt.grid(True, alpha=0.3)
