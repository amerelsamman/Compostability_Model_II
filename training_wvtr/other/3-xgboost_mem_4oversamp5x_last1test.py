...
parser.add_argument('--output', type=str, default='models/wvtr/v14/last1_testing',
                    help='Output directory path (default: models/wvtr/v14/last1_testing)')
...
# Convert categorical features to category dtype for native XGBoost handling
...
# Fill numerical features with 0, categorical features with 'Unknown'
for col in X.columns:
...
# No preprocessing pipeline needed - XGBoost will handle categorical features natively

# Split data ensuring last 22 blends are in training
print("Splitting data with 80/20 split, ensuring last 22 blends in training...")

# Put the very last molecule in test set (no leakage)
test_indices = [len(df) - 1]  # Only the very last molecule

# Last 22 blends minus the very last one (to avoid leakage)
last_22_indices = list(range(len(df) - 22, len(df) - 1))  # 2098 to 2118

# Create a mask for the remaining data (excluding last 22 + the test molecule)
remaining_indices = list(range(len(df) - 22))

# Use 80% of remaining data for training, 20% for testing
from sklearn.model_selection import train_test_split
train_remaining, _ = train_test_split(
    remaining_indices, 
    test_size=0.2, 
    random_state=42
)

# Combine training indices: remaining training data + last 21 blends (excluding the test molecule)
train_indices = train_remaining + last_22_indices

X_train, X_test = X.iloc[train_indices], X.iloc[test_indices]
y_train, y_test = y.iloc[train_indices], y.iloc[test_indices]

print(f"Last 21 blend indices (excluding test molecule): {last_22_indices}")
print(f"Training set size: {len(X_train)}")
print(f"Test set size: {len(X_test)}")
print(f"Train/test ratio: {len(X_train)/(len(X_train)+len(X_test)):.1%}/{len(X_test)/(len(X_train)+len(X_test)):.1%}")
print(f"Last 21 blends in training: {all(i in train_indices for i in last_22_indices)}")
print(f"Only the very last molecule in test set: {test_indices}")
print(f"Test molecule index: {test_indices[0]} (ensuring no data leakage)")

# Oversample last 4 blends in training set by 5x (since the very last molecule is in test set)
...
print(f"Oversampling last 4 training samples (indices {last_4_train_indices}) by 5x")
...
# Create XGBoost model with native categorical feature handling
model = xgb.XGBRegressor(
    n_estimators=150,       # Moderate number of trees
    max_depth=9,           # Higher depth for better memorization
...
categories = {
    ...
    'Environmental (T, RH)': ['Temperature (C)', 'RH (%)'],
...
# Create separate plot for last 22 blends performance
print("\n=== LAST 22 BLENDS PERFORMANCE ===")
# Get predictions for last 22 blends
last_22_X = X.iloc[last_22_indices]
last_22_y = y.iloc[last_22_indices]
last_22_pred = model.predict(last_22_X)

last_22_mae = mean_absolute_error(last_22_y, last_22_pred)
last_22_rmse = np.sqrt(mean_squared_error(last_22_y, last_22_pred))
last_22_r2 = r2_score(last_22_y, last_22_pred)

print(f"Last 22 Blends Performance:")
print(f"R² Score: {last_22_r2:.4f}")
print(f"Mean Absolute Error: {last_22_mae:.4f}")
print(f"Root Mean Squared Error: {last_22_rmse:.4f}")

# Get predictions for last 5 blends from the entire dataset (regardless of train/test split)
last_5_indices = list(range(len(df) - 5, len(df)))  # Last 5 blends from entire dataset
...
# Create separate plot for last 22 blends and last 5 blends - Log Scale
plt.figure(figsize=(20, 8))

# Plot 1: Last 22 blends - Log-transformed predictions
plt.subplot(2, 3, 1)
plt.scatter(last_22_y, last_22_pred, color='red', s=100, alpha=0.7, label=f'Last 22 Blends (MAE={last_22_mae:.2f}, RMSE={last_22_rmse:.2f})')
plt.plot([last_22_y.min(), last_22_y.max()], [last_22_y.min(), last_22_y.max()], 'k--', lw=2)
plt.xlabel('Actual Log(Property)')
plt.ylabel('Predicted Log(Property)')
plt.title('Last 22 Blends: Log-Transformed Predictions (Memorization Model)')
plt.legend()
plt.grid(True, alpha=0.3)

# Add individual data point labels
for i, (actual, pred) in enumerate(zip(last_22_y, last_22_pred)):
...
# Plot 3: Last 22 blends - Original scale predictions
plt.subplot(2, 3, 3)
original_actual_22 = np.exp(last_22_y)
original_pred_22 = np.exp(last_22_pred)
relative_errors_22 = np.abs(original_actual_22 - original_pred_22) / original_actual_22
relative_mae_22 = np.mean(relative_errors_22)
relative_rmse_22 = np.sqrt(np.mean(relative_errors_22**2))

original_mae_22 = mean_absolute_error(original_actual_22, original_pred_22)
original_rmse_22 = np.sqrt(mean_squared_error(original_actual_22, original_pred_22))

plt.scatter(original_actual_22, original_pred_22, color='red', s=100, alpha=0.7, 
           label=f'Last 22 Blends\nAbs MAE: {original_mae_22:.2f}\nAbs RMSE: {original_rmse_22:.2f}\nRel MAE: {relative_mae_22:.1%}\nRel RMSE: {relative_rmse_22:.1%}')
plt.plot([original_actual_22.min(), original_actual_22.max()], [original_actual_22.min(), original_actual_22.max()], 'k--', lw=2)
plt.xlabel('Actual Property (original scale)')
plt.ylabel('Predicted Property (original scale)')
plt.title('Last 22 Blends: Original Scale Predictions')
plt.legend()
plt.grid(True, alpha=0.3)
...
plt.subplot(2, 3, 5)
metrics = ['R²', 'MAE', 'RMSE']
last_22_metrics = [last_22_r2, last_22_mae, last_22_rmse]
last_5_metrics = [last_5_r2, last_5_mae, last_5_rmse]

x = np.arange(len(metrics))
width = 0.35

plt.bar(x - width/2, last_22_metrics, width, label='Last 22 Blends', color='red', alpha=0.7)
...
rel_last_22 = [relative_mae_22, relative_rmse_22]
rel_last_5 = [relative_mae_5, relative_rmse_5]

x = np.arange(len(rel_metrics))
plt.bar(x - width/2, rel_last_22, width, label='Last 22 Blends', color='red', alpha=0.7)
...
plt.tight_layout()
plt.savefig(os.path.join(args.output, 'last_22_and_5_blends_performance.png'), dpi=300, bbox_inches='tight')
plt.show()

# Calculate original scale metrics
original_mae_22 = mean_absolute_error(original_actual_22, original_pred_22)
original_rmse_22 = np.sqrt(mean_squared_error(original_actual_22, original_pred_22))
original_r2_22 = r2_score(original_actual_22, original_pred_22)

original_mae_5 = mean_absolute_error(original_actual_5, original_pred_5)
original_rmse_5 = np.sqrt(mean_squared_error(original_actual_5, original_pred_5))
original_r2_5 = r2_score(original_actual_5, original_pred_5)

print(f"\nLast 22 Blends - Original Scale Performance:")
print(f"R² Score: {original_r2_22:.4f}")
print(f"Mean Absolute Error: {original_mae_22:.4f}")
print(f"Root Mean Squared Error: {original_rmse_22:.4f}")
print(f"Relative MAE: {relative_mae_22:.4f} ({relative_mae_22*100:.2f}%)")
print(f"Relative RMSE: {relative_rmse_22:.4f} ({relative_rmse_22*100:.2f}%)")
