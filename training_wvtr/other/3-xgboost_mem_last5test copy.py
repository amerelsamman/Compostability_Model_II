...
parser.add_argument('--output', type=str, default='models/wvtr/v13/last5_testing',
                    help='Output directory path (default: models/wvtr/v13/last5_testing)')
...
# Split data ensuring last 22 blends are in training and last 5 blends are in testing
print("Splitting data with 80/20 split, ensuring last 22 blends in training and last 5 blends in testing...")

last_22_indices = list(range(len(df) - 22, len(df)))
last_5_indices = list(range(len(df) - 5, len(df)))  # Last 5 go to testing
last_17_indices = list(range(len(df) - 22, len(df) - 5))  # Remaining 17 of last 22 go to training

# Create a mask for the remaining data (excluding last 22)
...
# Combine training indices: remaining training data + last 17 blends
train_indices = train_remaining + last_17_indices
# Combine test indices: remaining test data + last 5 blends
test_indices = test_remaining + last_5_indices

X_train, X_test = X.iloc[train_indices], X.iloc[test_indices]
y_train, y_test = y.iloc[train_indices], y.iloc[test_indices]

print(f"Last 22 blend indices: {last_22_indices}")
print(f"Last 5 blend indices (testing): {last_5_indices}")
print(f"Last 17 blend indices (training): {last_17_indices}")
print(f"Training set size: {len(X_train)}")
print(f"Test set size: {len(X_test)}")
print(f"Train/test ratio: {len(X_train)/(len(X_train)+len(X_test)):.1%}/{len(X_test)/(len(X_train)+len(X_test)):.1%}")
print(f"Last 5 blends in testing: {all(i in test_indices for i in last_5_indices)}")
print(f"Last 17 blends in training: {all(i in train_indices for i in last_17_indices)}")
...
last_5_indices = last_22_indices[-5:]
